...
# === 7. inf 값 처리 (NEW) ===
# pct_change()에서 0으로 나눌 때 발생하는 inf 값을 NaN으로 변환
btc_data_with_indicators.replace([np.inf, -np.inf], np.nan, inplace=True)

# === 8. 최종 데이터 전처리 (기존 7번) ===
# 보조지표 NaN + pct_change/diff NaN + inf(->NaN) 모두 제거
...
# === 11. 모델 구성 (단순화된 GRU) ===
model = Sequential([
    GRU(32, activation='tanh', input_shape=(window_size, x_train.shape[2]), return_sequences=False),
    BatchNormalization(),
    Dropout(0.4),
...
callbacks = [
    ...
    ModelCheckpoint('best_model_GRU_classifier_v12.keras', monitor='val_accuracy', save_best_only=True, mode='max')
]
...
print("\n--- Classification Report (GRU - 1-Hour Ahead, Stationary) ---")
...
ax1.set_title('Model Accuracy (GRU v12 - Stationary Features)')
...
ax2.set_title('Model Loss (GRU v12 - Stationary Features)')
ax2.set_ylabel('Loss')
